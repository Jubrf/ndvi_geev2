...
# ✅ IMPORTANT : importer shapely_to_ee !!
...
st.title("🌱 NDVI – Analyse simple (SHP uniquement)")
...
# ============================================================
# ✅ DETECTION CRS SHP (méthode fiable)
# ============================================================
...
if crs_raw:
    st.success("✅ CRS détecté automatiquement depuis le fichier SHP")
    source_crs = pyproj.CRS.from_user_input(crs_raw)
    st.write("DEBUG CRS =", source_crs)
else:
    st.warning("⚠ Le SHP ne fournit PAS de CRS (pas de .prj).")
    epsg_code = st.text_input(
        "Entrez le code EPSG du fichier (ex : 3948 pour CC48, 2154 pour Lambert 93) :"
    )
    if not epsg_code:
        st.stop()

    try:
        source_crs = pyproj.CRS.from_epsg(int(epsg_code))
        st.success(f"✅ CRS défini : EPSG:{epsg_code}")
    except:
        st.error("❌ Code EPSG invalide")
        st.stop()
...
# ✅ extension AOI pour sécuriser la dalle Sentinel
expand = 0.1
...
if img is not None and d is not None:
    ...
    for feat in features:
        ...
        # ✅ Convertir géométrie SHP → ee.Geometry
        ...
        st.write("DEBUG pixels pour", num_ilot, ":", pixel_count)
