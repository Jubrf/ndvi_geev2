...
# ============================================================
# ✅ IMPORT UTILS — load_vector + gee_ndvi ORIGINAUX
# ============================================================
...
# DEBUG AOI
st.write("DEBUG AOI :", [minx-expand,miny-expand,maxx+expand,maxy+expand])
...
def tuile_selector(label, dates_key):
    ...
    mode = st.radio(f"Choisir la tuile ({label})",
    ...
    if mode=="Recherche par mois":
        year = st.selectbox(f"Année ({label})",
        ...
        month_num,_ = st.selectbox(f"Mois ({label})", months,
                                   key=f"month_{label}",
        ...
        if st.session_state.get(dates_key):
            selected = st.selectbox(f"Dates du mois ({label})",
                                    st.session_state[dates_key],
...
# ============================================================
# ✅ ANALYSE NDVI — VERSION ORIGINALE + DEBUG FOOTPRINT
# ============================================================
...
if img is not None and d is not None:
    ...
    for feat in features:
        geom = feat["geometry"]
        num = feat["properties"].get("NUM_ILOT","ILOT")

        # ✅ NDVI ZONAL — conversion interne correcte
        nd_mean, veg_prop = zonal_stats_ndvi(ndvi, veg_mask, geom)

        # ✅ DEBUG (valeurs finales)
        st.write(f"DEBUG NDVI brut pour {num} :", nd_mean)
...
if st.session_state.result_single is not None:
    ...
    for idx, feat in enumerate(features):
        ...
        tooltip=(f"<b>Ilot :</b> {df.iloc[idx]['NUM_ILOT']}<br>"
